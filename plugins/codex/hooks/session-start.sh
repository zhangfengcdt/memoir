...
# (agent-generated repo snapshot written by the memoir-onboard skill). Both are
# machinery, not user captures; counting them inflates the prompt-visible
...
# skips; users without a code repo can inspect branches with the memoir CLI.)
unmerged=""
...
# Concurrent-session warning: if another Codex session shares this
...
# Inject a short taxonomy snapshot as additionalContext so Codex sees what
...
#                       lessons), populated by the memoir-onboard skill
#                       on the code SHA.
#   - non-git folder  → project:onboard (file-shape: summary, structure tree,
#                       per-file blobs), populated by the memoir-onboard skill
...
# emit a one-line hint nudging the user to populate it with memoir-onboard.
if [ "${MEMOIR_ONBOARD_INJECT:-1}" = "1" ]; then
  ...
  if [ -z "$onboard_block" ]; then
    ...
    if [ "$USER_MEMORIES" != "0" ]; then
      ...
      onboard_block+="(none yet - invoke the memoir-onboard skill to generate one; future sessions will auto-inject it here)"
    fi
  fi
...
# Refresh the lightweight memory-count cache used by UserPromptSubmit.
write_memory_count_cache "$USER_MEMORIES" || true
