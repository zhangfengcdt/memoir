...
# asks Codex exec for a one-line summary,
...
# turn has no file edits or when Codex exec is unavailable. Independent of
...
if [ "${MEMOIR_NO_CODE_SUMMARY:-}" != "1" ]; then
  ...
  if [ -n "$EDITS_JSON" ]; then
    # Build the model input: an optional [User prompt] header (the *why*) followed
    # by "<tool> <file_path>\n<snippet>\n---" per edit. Capped at ~8KB so multi-file
    # refactors stay well under context. The user prompt gives Codex intent it
...
# One-shot extract + classify via Codex exec. Output format: <taxonomy-path><TAB><fact>
# per line. We then call `memoir remember --path <path>` for each line, which
# bypasses memoir's internal LLM classifier entirely. Net result: ONE model call
...
# manual `memoir remember "fact"` (without -p). Falls back to a hardcoded hint
...
FACTS_TSV=$(codex_exec_prompt "$STOP_SYSTEM_PROMPT" "$PARSED" 2>/dev/null || true)
...
# memoir's classifier LLM chain. The line-format check guards against the model
# going off-format (returning preamble, missing tabs, etc.). Comma-separated paths
...
# Refresh the lightweight memory-count cache so the count ticks up after this turn's
# captures. Stop runs after the turn, so a small extra CLI round-trip here is fine.
NEW_COUNT=$(compute_user_memory_count 2>/dev/null || true)
if [ -n "$NEW_COUNT" ]; then
  write_memory_count_cache "$NEW_COUNT" || true
fi
