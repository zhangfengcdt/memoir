...
# accumulates across two turns. Suppresses the haiku-driven capture and
...
#
# Usage: bash plugins/claude-code/tests/test_stop_code_summary.sh
...
# Keep this test deterministic: don't let the haiku-driven capture or summary
...
# logic without invoking haiku. Takes one summary string, appends to KEY.
...
")
  memoir --json -s "$STORE" remember "$merged" -p "$KEY" >/dev/null 2>&1
}
