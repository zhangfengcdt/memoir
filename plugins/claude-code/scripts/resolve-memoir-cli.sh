...
#                      "uvx --from memoir-ai memoir". Empty if no working
#                      invocation exists. Hooks use `[ -z "$MEMOIR_CMD" ]`
#                      as the "is the CLI available?" check.
#
...
#      Fastest cold start, no warmup.
#   2. `uvx --from memoir-ai memoir` — uv installed, no global memoir.
#      Ephemeral, no Python env pollution, ~1s warmup on first use then
#      cached.
#   3. `uv tool run --from memoir-ai memoir` — uv installed but uvx shim
#      missing (older uv layouts).
#   4. Empty — caller must surface an install hint.
#
# Hooks add common user bin paths to PATH before sourcing this; standalone
# scripts (slash-command shims) should do the same if they want to find a
