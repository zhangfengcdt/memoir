---
name: memory-recall
description: "Recall relevant facts from past sessions via memoir. Use when prior context (preferences, past decisions, conventions from previous sessions) could shape the current turn. Typical triggers: (1) QUESTIONS — 'what did I decide about X', 'why did we do Y', 'have I seen this before'; (2) TASK / DESIGN / IMPLEMENTATION PROMPTS that explicitly reference past decisions or established preferences — 'follow our usual pattern for X', 'match the existing Y style', 'use the same approach as last time'. Also use when you see `[memoir] memory available` hints injected via SessionStart or UserPromptSubmit. If the task likely depends on prior preferences (style, tooling, naming conventions, architectural decisions), recall first — silently applying remembered preferences is a common failure mode. Skip when: (a) the prompt is trivial (single-file read, simple lookup); (b) the question is purely about current code state (use Read/Grep); (c) the task is ephemeral (today's work only, one-off scripts); (d) no plausible prior context exists (new-feature scaffolding in an area with no history, greenfield work); (e) the prompt is a pure UI/frontend tweak with no policy implications; (f) the user has asked to ignore memory."
context: fork
allowed-tools: Bash
---
