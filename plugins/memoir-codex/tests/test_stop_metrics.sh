...
#
# Stages a tmp memoir store + a fixture Codex JSONL transcript, invokes stop.sh
...
export MEMOIR_NO_CAPTURE=1  # only test metrics; skip model/capture path
cd "$PROJ"
...
# Build a fixture Codex JSONL transcript with one realistic turn.
fixture_transcript() {
  local out_path="$1"
  python3 - "$out_path" <<'PY'
import json, sys
...
fixture_transcript "$TRANSCRIPT"
INPUT_JSON=$(printf '{"transcript_path":"%s","stop_hook_active":false}' "$TRANSCRIPT")
MEMOIR_NO_METRICS=1 bash -c "echo '$INPUT_JSON' | bash '$PLUGIN_ROOT/hooks/stop.sh'" >/dev/null 2>&1 || true
