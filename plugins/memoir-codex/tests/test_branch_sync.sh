...
#
# Exercises session-start.sh directly (no Codex needed) against a
...
session_start_status() {
  bash "$PLUGIN_ROOT/hooks/session-start.sh" </dev/null 2>&1 |
...
if [ -f "$STORE/.git/plugin-sticky-branch" ]; then
  # The plugin's checkout helper also clears it; simulate that path too.
...
# restarting Codex. UserPromptSubmit and Stop hooks must re-run
...
# User git-switches to feature/b without restarting Codex.
...
echo "$INPUT" | bash "$PLUGIN_ROOT/hooks/user-prompt-submit.sh" >/dev/null 2>&1
...
memoir -s "$STORE" checkout feature/a >/dev/null
# Stop's model path may not be reachable without a credentialed `codex` CLI, but
# the auto-match call happens before the model extraction — so even if the
# capture ultimately no-ops, the branch switch is what we're verifying here.
echo "{\"transcript_path\":\"$TRANSCRIPT\"}" | bash "$PLUGIN_ROOT/hooks/stop.sh" >/dev/null 2>&1
