#!/usr/bin/env python3
"""Prompt test harness for the Codex plugin.
...
each test case against a real LLM via ``codex exec`` (Codex auth, no API key
needed).
...
with your own eyes:
  ...
  <run>/<prompt>/<case>/input.txt    ← input appended to codex exec
...
class CaseSpec:
    ...
    input: str
    expect: list[dict]
    # Case mode. "llm" = run input through codex exec against the named
...
def _render_command_sh(model: str, system_prompt_path: str, input_path: str) -> str:
    return f"""\
#!/usr/bin/env bash
# Replay this case against codex exec — same env vars and flags as the harness used.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd)"
SYSTEM_PROMPT=$(cat "$SCRIPT_DIR/{system_prompt_path}")
INPUT=$(cat "$SCRIPT_DIR/{input_path}")
OUT="$SCRIPT_DIR/output.replay.txt"
{{ printf '%s\\n\\n=== Input ===\\n' "$SYSTEM_PROMPT"; printf '%s' "$INPUT"; }} | \\
...
def _invoke_codex(model: str, system_prompt: str, case_input: str) -> tuple[str, str, int]:
    """Return (last_message, stderr, returncode). Raises RuntimeError if codex missing."""
    import shutil
    import tempfile

    if not shutil.which("codex"):
        raise RuntimeError(
            "`codex` CLI not found on PATH. Install Codex and run `codex login` first."
        )
    env = {
        **os.environ,
        "MEMOIR_NO_CAPTURE": "1",
    ...
    try:
        ...
        try:
            _, stderr = proc.communicate(input=prompt, timeout=180)
        ...
        return stdout, stderr or "", proc.returncode or 0
    finally:
...
"""
    for k, v in case.env.items():
        if k == "USER_MEMORIES":
            # Memory-count cache — rewrite per gate.py's contract.
            replay += (
                f'mkdir -p "$MEMOIR_STORE/.git" && '
                f"printf '%s\\n' {shlex.quote(str(v))} "
                f'> "$MEMOIR_STORE/.git/plugin-memory-count-cache"\n'
            )
        else:
...
def _run_single(
    ...
    *,
    ...
    # Real LLM call.
    try:
        stdout, stderr, rc = _invoke_codex(model, assembled.system_prompt, case.input)
    except subprocess.TimeoutExpired:
        notes.append("codex exec timed out (180s)")
    ...
    if rc != 0:
        notes.append(f"codex exec exited {rc}")
...
def main() -> int:
    p = argparse.ArgumentParser(
        prog="runner.py",
        description="Prompt test harness for the Codex plugin.",
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    common_model = ("--model", {"required": True, "help": "Codex model name (for smoke validation use gpt-5.4)"})
