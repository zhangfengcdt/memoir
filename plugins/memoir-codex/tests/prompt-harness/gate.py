...
The hook reads its input from stdin as a JSON string (production: Codex
passes ``{"prompt": "...", ...}``). It reads ``USER_MEMORIES`` from
``<store>/.git/plugin-memory-count-cache``. Both are simulated here.
"""
...
def run_gate(
    ...
    ``store_dir`` is used for two things:
      ...
      * The harness writes ``USER_MEMORIES`` (when supplied via env) into
        ``<store>/.git/plugin-memory-count-cache`` so the hook reads the
    ...
    # memory-count cache file. If the case asked for a count, materialize it
    # there so the hook's real read path exercises.
    if env and "USER_MEMORIES" in env and store_dir is not None:
        cache = store_dir / ".git" / "plugin-memory-count-cache"
