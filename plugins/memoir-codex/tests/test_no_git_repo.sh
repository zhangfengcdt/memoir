...
run_session_start() {
  bash "$PLUGIN_ROOT/hooks/session-start.sh" </dev/null 2>/dev/null || true
}
...
assert_eq "[1.3] in_git_repo helper sees non-git" "no" \
  "$(bash -c 'source "$PLUGIN_ROOT/hooks/common.sh" >/dev/null 2>&1; if in_git_repo; then echo yes; else echo no; fi')"
assert_eq "[1.4] code_git_branch returns empty" "" \
  "$(bash -c 'source "$PLUGIN_ROOT/hooks/common.sh" >/dev/null 2>&1; code_git_branch')"
...
post_branch=$(bash -c 'source "$PLUGIN_ROOT/hooks/common.sh" >/dev/null 2>&1; auto_match_memoir_branch || true; memoir_json status | python3 -c "import json,sys; print(json.loads(sys.stdin.read())[\"branch\"])"')
...
EXTRACTORS="$PLUGIN_ROOT/skills/memoir-onboard/extractors.py"
