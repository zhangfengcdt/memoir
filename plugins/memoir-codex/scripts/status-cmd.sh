#!/usr/bin/env bash
# Backing script for documented Codex status snippets.
#
...
# `memoir`, which broke status snippets on machines that only had `uv`
# installed.
set -e
...
# installed memoir mid-session and then ran a status snippet without
# restarting Codex) the store directory won't exist. Materialize it
# now so the status command works instead of erroring "Store not found".
