#!/usr/bin/env bash
# Shared setup for memoir Codex command hooks. Sourced by every hook.
#
...
# Project directory: git root preferred. Codex hook commands run with the session
# cwd, but the JSON payload also carries cwd; prefer the payload so tests and
# nested invocations can be explicit.
...
SCRIPT_PARENT_FALLBACK="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT_PARENT="${PLUGIN_ROOT:-$SCRIPT_PARENT_FALLBACK}"
export PLUGIN_ROOT="$SCRIPT_PARENT"

HOOK_CWD=$(_json_val "$INPUT" "cwd" "$(pwd)")
MEMOIR_SESSION_ID=$(_json_val "$INPUT" "session_id" "${PPID:-$$}")
MEMOIR_HOOK_MODEL=$(_json_val "$INPUT" "model" "${MEMOIR_CODEX_MODEL:-}")

_GIT_ROOT="$(cd "$HOOK_CWD" 2>/dev/null && bash "$SCRIPT_PARENT/scripts/derive-store-path.sh" --print-git-root 2>/dev/null || echo "")"
if [ -n "$_GIT_ROOT" ]; then
  _PROJECT_DIR="$_GIT_ROOT"
else
  _PROJECT_DIR="$HOOK_CWD"
fi
...
# and the rationale live in scripts/resolve-memoir-cli.sh so skills and
...
ensure_store() {
  ...
  # helper scripts and SessionStart use the same bootstrap path.
...
# record_branch_synced <branch> — called after a successful sync-branch
...
")
  ...
  while IFS= read -r b; do
    ...
    # sync-marker timestamp recorded by sync-branch.
    #
...
# write_memory_count_cache <user_memory_count> — persist the current user-facing
# memory count so UserPromptSubmit can render a cheap memory-available hint.
write_memory_count_cache() {
  local count="$1"
  [ ! -d "$MEMOIR_STORE_PATH/.git" ] && return 0
  printf '%s\n' "$count" > "$MEMOIR_STORE_PATH/.git/plugin-memory-count-cache" 2>/dev/null || true
}
...
# and stop so both surface the same number in the cheap prompt cache.
...
# codex_exec_prompt <system_prompt> <input_text>
#
# Codex exec has no dedicated system-prompt flag, so the hook sends a single
# instruction block followed by the turn text. The output-last-message file is
# used so banners, event logs, and token summaries do not leak into parsers that
# expect prompt-harness TSV or one-line summaries.
codex_exec_prompt() {
  local system_prompt="$1" input_text="$2"
...

  {
    ...
    printf '=== Input ===\n'
    printf '%s' "$input_text"
  } | MEMOIR_NO_CAPTURE=1 MEMOIR_NO_METRICS=1 MEMOIR_NO_CODE_SUMMARY=1 \
...
write_session_heartbeat() {
  ...
  session_id="${MEMOIR_SESSION_ID:-${PPID:-$$}}"
...
concurrent_session_warning() {
  ...
  dir=$(_heartbeats_dir)
  [ ! -d "$dir" ] && return 0
  my_session="${MEMOIR_SESSION_ID:-${PPID:-$$}}"
  current=$(memoir_json status 2>/dev/null | python3 -c "import json,sys; print(json.loads(sys.stdin.read() or '{}').get('branch',''))" 2>/dev/null)
  ...
  if [ -n "$other_branch" ]; then
    printf 'warning: concurrent session detected on branch %s. Captures may collide; set a distinct MEMOIR_STORE per session.' "$other_branch"
  fi
}
...
if 'files' in cfg['suppressed_roots']:
    ...
    if file_count:
        print(f'files: {file_count} indexed (invoke memoir-onboard for per-file detail)')

print(f'</{cfg[\"tag\"]}>')
if stale:
    print('(snapshot is stale - invoke memoir-onboard to refresh)')
