...
class TestRealisticLLMExpansion:
    ...
    async def test_expand_web_development_taxonomy(self, taxonomy_with_realistic_llm):
        ...
        assert frontend_node.is_dynamic == True
    ...
    async def test_expand_professional_relationships(self, taxonomy_with_realistic_llm):
        ...
        with patch.object(taxonomy, '_call_llm', new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = LLM_TEST_DATA["relationships_professional_types"]["response"]
            
            result = await taxonomy.expand_subtree_with_llm("relationships.professional.other")
        ...
        assert "relationships.professional.other.open_source_collaborators" in result.new_paths
    
    @pytest.mark.asyncio
    async def test_parallel_expansion_multiple_domains(self, taxonomy_with_realistic_llm):
    ...
    def test_prompt_generation_follows_paper_format(self, taxonomy_with_realistic_llm):
        ...
        context = ExpansionContext(
            ...
            sibling_categories=["programming", "databases", "networking", "systems", "algorithms"],
            unclassified_items=[
    ...
    @pytest.mark.asyncio
    async def test_reclassification_with_realistic_categories(self, taxonomy_with_realistic_llm):
        ...
        node = taxonomy.path_index["knowledge.technical.other"]
        node.other_items = [
            {"content": "React component lifecycle tutorial", "metadata": {"type": "frontend"}},
            {"content": "Express.js middleware for authentication", "metadata": {"type": "backend"}},
            {"content": "Machine learning model deployment", "metadata": {"type": "ml"}},
            {"content": "GraphQL resolver optimization", "metadata": {"type": "api"}},
            {"content": "WebSocket connection handling", "metadata": {"type": "realtime"}},
        ]
    ...
    def test_apply_location_technology_combinations(self, taxonomy_with_realistic_llm):
        ...
        technologies = ["artificial_intelligence", "blockchain", "cloud_computing", "cybersecurity"]
        
        for loc in locations:
            taxonomy._add_path_to_tree(taxonomy.root, f"location.{loc}", is_dynamic=False)
        
        for tech in technologies:
            taxonomy._add_path_to_tree(taxonomy.root, f"technology.{tech}", is_dynamic=False)
        ...
        combination = TaxonomyCombination(
            ...
            examples=["ai_in_silicon_valley", "blockchain_in_london", "cloud_in_seattle"]
        )
        ...
        created_combinations = [p for p in possible_combinations if p in taxonomy.path_index]
        assert len(created_combinations) > 0
    
    def test_expansion_statistics_with_realistic_data(self, taxonomy_with_realistic_llm):
        ...
        expansions = [
            TaxonomyExpansionResult(
                new_paths=["knowledge.technical.other.frontend", "knowledge.technical.other.backend"],
                migrated_items=15,
                suggested_paths=["knowledge.technical.other.devops"],
                reasoning="Expanded web development categories based on 20 items"
            ),
            TaxonomyExpansionResult(
                new_paths=["goals.career.other.leadership", "goals.career.other.entrepreneurship"],
                migrated_items=8,
                suggested_paths=[],
...
class TestRealisticMockLLM:
    ...
    async def test_mock_llm_with_random_subset(self):
        ...
        responses = []
        for _ in range(5):
            response = await llm.generate("knowledge.technical.other with React and Vue items")
