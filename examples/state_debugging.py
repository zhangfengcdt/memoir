...
async def main():
    ...
    try:
        ...
        await prolly_store.store_memory_async(
            namespace, "User likes coffee in the morning", "preferences.daily.coffee"
        )
        ...
        commit_history.append(
            (corrupted_commit, timestamp_corrupted, "❌ Corrupted memory")
        )
        ...
        corrupt_check = prolly_store.get(
            (namespace,), "preferences.programming.hate_all"
        )
        ...
        corrupt_exists = prolly_store.get(
            (namespace,), "preferences.programming.hate_all"
        )
        if corrupt_exists is not None:
            print(
                f"  ❌ Corrupted state ({corrupted_commit}): Contains 'hate all languages'"
            )
        ...
        print("-" * 60)
        for i, (commit_id, timestamp, description) in enumerate(
            reversed(commit_history), 1
        ):
