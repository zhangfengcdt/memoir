...
async def main():
    ...
    try:
        ...
        # Build realistic production history: 4 weeks x 7 days x 3 interactions = 84 base memories
        ...
        production_timeline.append(
            (
                initial_checkpoint,
                initial_time,
                f"📊 Production checkpoint: {initial_count} memories",
                "checkpoint",
            )
        )
        ...
        await prolly_store.store_memory_async(
            namespace, "User prefers minimal UI designs", "preferences.ui.style.minimal"
        )
        day1_time = datetime.now()
        day1_commit = f"day1_{int(time.time())}"
        prolly_store.create_time_snapshot(day1_commit)
        production_timeline.append(
            (day1_commit, day1_time, "📋 User sets minimal UI preference", "normal")
        )
        ...
        await prolly_store.store_memory_async(
            namespace, "User likes dark mode for coding", "preferences.ui.theme.dark"
        )
        day2_time = datetime.now()
        day2_commit = f"day2_{int(time.time())}"
        prolly_store.create_time_snapshot(day2_commit)
        production_timeline.append(
            (day2_commit, day2_time, "🎨 User adds dark mode preference", "normal")
        )
        ...
        production_timeline.append(
            (
                problem_commit,
                problem_time,
                "❌ Agent recommends neon colors (BUG)",
                "error",
            )
        )
        ...
        production_timeline.append(
            (
                "USER_REPORT",
                report_time,
                "📧 User complaint: 'terrible colors yesterday'",
                "report",
            )
        )
        print(
            "  📧 10:00 - User reports: 'Agent recommended terrible colors yesterday at 11:15 AM'"
        )
        ...
        print(
            f"   Traditional approach: Search through {final_count} memories manually ❌"
        )
        ...
        print(
            f"   🎯 Time-traveled back through {final_count - problem_count} memories instantly!"
        )
        ...
        initial_count_check = len(
            [
                m
                for m in prolly_store.search((namespace,), limit=300)
                if m[2] is not None
            ]
        )
        print(
        ...
        print(
            f"  🎯 Debugged by time-traveling through {final_count} memories in seconds!"
        )
        ...
        fix_verification = prolly_store.get(
            (namespace,), "agent.recommendations.corrected_ui"
        )
        if fix_verification:
        ...
        production_timeline.append(
            (deployed_commit, deployed_time, "✅ Production fix deployed", "fix")
        )
        ...
        print("=" * 70)
        for i, (commit_id, timestamp, description, event_type) in enumerate(
            reversed(production_timeline), 1
        ):
        ...
        print("  • Memoir debugging: Instant time-travel to exact moments")
        print("  • Perfect for production systems with extensive memory history")
