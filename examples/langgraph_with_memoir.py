...
async def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        agent_namespace = create_memory_namespace(
            agent_id="assistant", thread_id="conversation_001"
        )
        ...
        memories = [
            {
                "key": "user_info",
                "content": "User is a data scientist working with Python",
                "type": "profile",
            },
            {
                "key": "project",
                "content": "Building a machine learning pipeline for CSV processing",
                "type": "context",
            },
            {
                "key": "tools",
                "content": "User prefers pandas and scikit-learn",
                "type": "preference",
            },
        ]
        ...
        search_results = await langgraph_store.asearch(
            namespace=agent_namespace, query="machine learning CSV pandas", limit=3
        )
        ...
        async def process_with_memory(state: ConversationState) -> ConversationState:
            ...
            if last_msg and isinstance(last_msg, HumanMessage):
                ...
                memories = await langgraph_store.asearch(
                    namespace=agent_namespace, query=last_msg.content, limit=2
                )
